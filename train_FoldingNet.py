...
parser.add_argument('--outf', type=str, default='experiment',  help='output folder')
parser.add_argument('--model', type=str, default = '',  help='model path')

opt = parser.parse_args()
opt.nepoch = 25            #colab test
print(opt)
...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 100 == 0:
            ...
            print('[%d: %d/%d] %s test loss: %f middle test loss: %f' %(epoch, i, num_batch, blue('test'), loss.item(), mid_loss.item()), file=loss_log)
            sum_step = 0
            sum_loss = 0
            sum_mid_loss = 0
